...
const CreateDrawer: FC<IProps> = ({ visible, onClose, reload, secName }) => {
  ...
  useEffect(() => {
    form.setFieldsValue({ secName });
    getAllSec();
  }, [secName]);
  const onFinish = async (e: any) => {
    const res: any = await getUrl(`${baseURL}/admin/setMax`, "POST", {
      ...e,
      secName,
    });
    if (res.status === 200) {
    ...
    >
      <Form onFinish={onFinish} form={form}>
        ...
        <Form.Item {...formItemLayout} label="时间段类型">
          <Select onChange={setFullTime} defaultValue={fullTime}>
            <Option value={true}>全时间段</Option>
            <Option value={false}>可选时间段</Option>
          </Select>
        </Form.Item>
        {!fullTime && (
          <Form.Item {...formItemLayout} name="time" label="时间段">
            <Select>
